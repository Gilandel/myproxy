...
public class RequestHandler implements Runnable {
	 ...
	 * HTTPS Reference to this is required so it can be closed once completed.
	 */
	...
	public void run() {
		...
		try {
			...
			if (requestString != null) {
				...
				if (!urlString.substring(0, 5).equals("https")) {
					String temp = "https://";
					urlString = temp + urlString;
				} else if (!urlString.substring(0, 4).equals("http")) {
					String temp = "http://";
					urlString = temp + urlString;
				}
			...
			LOG.info("Request '{}' handled in: {}\n", urlString, Proxy1.getTime(System.currentTimeMillis() - start));
		}
	}
	...
	private void sendCachedPageToClient(File cachedFile) {
		...
		try {
			...
			// Standard text based file requested
				...
				try (BufferedReader cachedFileBufferedReader = new BufferedReader(
	...
	private void sendNonCachedToClient(String urlString) {
		...
		try {
			...
			File fileToCache = new File(Proxy1.cacheDir, fileName);
			...
			try (BufferedWriter fileToCacheBW = new BufferedWriter(new FileWriter(fileToCache))) {
				...
				// File is a text file
					...
					try (BufferedInputStream proxyToServerBuf = new BufferedInputStream(
						...
						line = new String(proxyToServerBuf.readAllBytes());
				...
				if (caching) {
					...
					fileToCacheBW.flush();
					Proxy1.addCachedPage(urlString, fileToCache);
				}
			}
	...
	private void handleHTTPSRequest(String urlString) {
		...
		String url = urlString.substring(8);
		...
		try {
			...
			String line;
			while ((line = proxyToClientBr.readLine()) != null && !"".equals(line)) {
				LOG.info(line);
			}
			...
			try (Socket proxyToServerSocket = new Socket(proxy)) {
				proxyToServerSocket.setSoTimeout(30_000);
				// proxyToServerSocket.set

				// Send Connection established to the client
				line = "HTTP/1.0 200 Connection established\r\n" + //
						"Proxy-Agent: ProxyServer/1.0\r\n" + //
						"\r\n";
				...
				try (BufferedWriter proxyToServerBW = new BufferedWriter(
